...
export interface ProgressiveScenario {
    id: string;
    title: string;
    scenario: string;
    context: string;
...
export class ProgressiveLearningAgent {
    ...
    constructor() {
        // Don't load memory in constructor to avoid SSR issues
        // Memory will be loaded lazily when needed
    }

    /**
     * Ensure memory is loaded (lazy loading for SSR compatibility)
     */
    ...
    async generateInitialScenario(
        ...
        const systemPrompt = `You are a senior ${domain.toUpperCase()} mentor creating an immersive workplace simulation scenario.

Create a realistic workplace scenario that simulates a ${domain === 'ai' ? 'Machine Learning/AI project situation' : 'Cloud infrastructure challenge'}.

RESPONSE FORMAT (return as JSON):
{
    "id": "unique_scenario_id",
    "title": "Engaging scenario title",
    "scenario": "Detailed workplace situation description with characters, context, and background",
    "context": "Technical background and requirements the learner needs to address",
    "expectedOutcome": "What a successful response should demonstrate or achieve",
    "difficulty": 3,
...
1. Create a story-driven workplace simulation (like scene1)
2. Include specific characters, company context, and realistic constraints
3. Present a clear challenge that requires both technical and business thinking
4. Make it engaging and relatable to real-world situations
5. Ensure the scenario can be evaluated objectively

STUDENT PROFILE:
- Domain: ${domain === 'ai' ? 'Artificial Intelligence/Machine Learning' : 'Cloud Computing'}
- Level: ${level}
- This is their first scenario - baseline assessment

DOMAIN-SPECIFIC REQUIREMENTS:
${domain === 'ai' ? `
AI/ML SCENARIO ELEMENTS:
- Realistic data science/ML project challenge
- Business stakeholders with specific needs
- Technical constraints and requirements
- Data quality, model selection, or deployment challenges
- Communication between technical and non-technical teams

Example context: New hire at a tech company, given first ML project, needs to present approach to team lead
` : `
CLOUD COMPUTING SCENARIO ELEMENTS:
- Infrastructure or deployment challenge
- Cost optimization and scalability requirements
- Security and compliance considerations
- Cross-team collaboration and communication
- Real-world constraints (budget, timeline, regulations)

Example context: New cloud engineer, company migration project, needs to propose solution architecture
`}

DIFFICULTY FOR ${level.toUpperCase()} LEVEL:
- Focus on fundamental concepts and basic problem-solving
- Clear guidance on what to consider
- Realistic but not overwhelming complexity
- Allow for learning-oriented responses

Make it engaging, realistic, and perfectly calibrated for initial assessment. Return ONLY valid JSON.`;
...
Be encouraging but honest, specific but supportive.`;
    ...
    private updateUserMemory(
        ...
        this.storeResponseData(memory, scenario, userResponse, feedback);
    ...
    private storeResponseData(
        ...
        // Store in localStorage as a learning document
        const document = {
        ...
        const existingDocs = JSON.parse(localStorage.getItem('futureflex_learning_docs') || '[]');
        existingDocs.push(document);
        localStorage.setItem('futureflex_learning_docs', JSON.stringify(existingDocs));

        // Create markdown content for GitHub export
        const markdownContent = `# Learning Session: ${scenario.title}
...
### Skill Gaps
${feedback.skillGaps.map(g => `- ${g}`).join('\n')}

## Session Context
- **Previous Scenarios:** ${memory.sessionHistory.length}
- **Overall Progress:** ${this.calculateAverageScore(memory)}%
- **Confidence Level:** ${feedback.confidenceLevel}/10
- **Ready for Next:** ${feedback.readinessForNext ? 'Yes' : 'No'}
...
`;
        ...
        // Store markdown separately for easy GitHub export
        const markdownDocs = JSON.parse(localStorage.getItem('futureflex_markdown_docs') || '[]');
        markdownDocs.push({ id: document.id, markdown: markdownContent });
        localStorage.setItem('futureflex_markdown_docs', JSON.stringify(markdownDocs));
    }
...
export const progressiveLearningAgent = {
    _instance: null as ProgressiveLearningAgent | null,
    getInstance(): ProgressiveLearningAgent {
        if (!this._instance) {
            this._instance = new ProgressiveLearningAgent();
        }
